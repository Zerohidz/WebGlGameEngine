 ...
 * Supports: vertices (v), normals (vn), faces (f)
 */
export class OBJLoader {
  ...
  static parse(gl: WebGL2RenderingContext, content: string): Geometry {
    ...
    const faces: Array<Array<{ v: number; vn: number }>> = [];
    ...
    for (const line of lines) {
      ...
      if (type === 'v') {
        ...
        // We'll support: v//vn format (position + normal, no texture coords)
        const face: Array<{ v: number; vn: number }> = [];
    ...
    const vertexMap = new Map<string, number>(); // Map "v/vn" -> vertex index
    let currentIndex = 0;
